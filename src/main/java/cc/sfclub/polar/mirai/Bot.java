...
import cc.sfclub.polar.mirai.packet.message.client.CGroupQuote;
...
public class Bot extends cc.sfclub.polar.wrapper.Bot {
    ...
    @Override
    public long sendMessage(long gid, String textMessage) {
        CGroupPlain msg = new CGroupPlain();
        msg.setSessionKey(Main.getSession());
        ArrayList<MessageChain> msgs = new ArrayList<>();
        String[] sp = spilt(textMessage);
        for (String s1 : sp) {
    ...
    public long sendMessage(TextMessage textMessage, String s) {
        CGroupQuote msg = new CGroupQuote();
